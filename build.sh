...
export KBUILD_BUILD_USER=A32 5G
export KBUILD_BUILD_HOST=A32is5G
...
if [ -f $GZIP ]; then
    ...
    [ ! -d $ANYKERNEL_DIR ] && git clone https://github.com/rufnx/AnyKernel3 -b a32x $ANYKERNEL_DIR
    ...
    ZIP_NAME=A326B-$(date +%Y%m%d-%H%M).zip
    cd $ZIP_DIR
